...
from my_msg_module_py3 import downscale
from my_msg_module_py3 import make_figure
...
#def make_test_image(imgarr, png_file):
...
def read_HRW(sat, time_slot, ntimes, dt=5, read_basic_or_detailed='DS', 
    ...
    data = create_seviri_scene(time_slot, in_msg.base_dir_ctth, sat=MSG_str, reader='nwcsaf-geo', file_filter=read_basic_or_detailed)  
    data.load(['HRW'])
    ...
    for it in range(1,ntimes):
        ...
        data_i = create_seviri_scene(time_slot_i, in_msg.base_dir_ctth, sat=MSG_str, reader='nwcsaf-geo', file_filter=read_basic_or_detailed)  
        data_i.load(['HRW'])
...
def get_cosmo_filenames ( t_sat, nrt=True, runs_before=0, cosmoDir="./cosmo/" ):
    ...
    t_run.strftime(in_msg.nowcastDir +"/"+ "%Y%m%d%H_")
    
    cosmo_file1 = t_run.strftime("%Y%m%d%H_")+hour_forecast1+"_"+cosmo+"_UV*.nc"
    cosmo_file2 = t_run.strftime("%Y%m%d%H_")+hour_forecast2+"_"+cosmo+"_UV*.nc"

    return cosmoDir+cosmo_file1, cosmoDir+cosmo_file2


def interpolate_cosmo(in_msg_datetime, layers, zlevel='pressure', area='ccs4', cosmo=None, nrt=False, rapid_scan_mode_satellite=True, cosmoDir="./cosmo/"):
    ...
    file1, file2 = get_cosmo_filenames ( in_msg_datetime, nrt=nrt, cosmoDir=cosmoDir )
    ...
    if len(filename1)<1 or len(filename2)<1:
        ...
        file1, file2 = get_cosmo_filenames ( in_msg_datetime, runs_before = 1, cosmoDir=cosmoDir )
        print(file1, file2)
        ...
        if len(filename1)>1 or len(filename2)>1:
            ...
            print("*** Error, no cosmo wind data for time: ", str(in_msg_datetime))
            print(file1)
            print(file2)
            quit()
    ...
    position_t = (in_msg_datetime.minute+dt)/5
...
def calculate_displacement( u_d, v_d, n_levels, size_x, size_y, ForecastTime, NumComputationSteps):
    ...
    #for level in [0]:
          ...
          for num_compSteps in range(NumComputationSteps):
                xy2 = particles_displacement(u_func=v*(-1), v_func=u, method=method, dt=dt*60,pts=xy2,
                                             size_x=size_x, size_y=size_y, wind_source=wind_source) #u_func=v*(-1)
                c = 0
...
#the type of tracking desired, uncomment one
    ...
    if len(sys.argv) > 7:
        # specify the loop end time for producing BT forecast 
        if len(sys.argv) == 12:
    ...
    if not exists(in_msg.nowcastDir):
        makedirs(in_msg.nowcastDir)
    ...
    while in_msg.datetime <= time_slotSTOP:
          ...
          if wind_source=="HRW":
              ...
              u_d, v_d = interpolate_cosmo(in_msg.datetime, layers, zlevel, area, nrt=in_msg.nrt, rapid_scan_mode_satellite=True, cosmoDir=in_msg.cosmoDir)
          else:
              print("*** Error in main ("+current_file+")")
                  ...
                  for rgb_num in range(len(rgbs)):
                      ...
                      if t*ForecastTime == dt_forecast1 or t*ForecastTime == dt_forecast2:
                          ...
                          if level == (n_levels-1):
                            ...
                            # in_msg.datetime.strftime( in_msg.nowcastDir )
                            outputFile = in_msg.datetime.strftime(in_msg.nowcastDir +"/"+ "%Y%m%d_%H%M")+"_"+rgb+"_t"+str(t*ForecastTime)+".p"
                            #outputFile = in_msg.nowcastDir +"/"+ "%s_%s_%s_t%s.p" % (yearS+monthS+dayS,hourS+minS,rgb,str(t*ForecastTime))
                            ...
                            if plot_DisplMeter:
                                ...
                                outputFig = "./plots/%s_%s_t%s_%s_DisplMeter_%s.png"%(rgb,yearS+dayS+hourS+minS,time_string,method,name_to_save) #time_string,
